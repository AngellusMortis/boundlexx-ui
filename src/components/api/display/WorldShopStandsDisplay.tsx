...
const mapState = (state: RootState) => ({
    ...
    locale: null,
    operationID: "listWorldShopStands",
    name: "Shop Stand",
...
function copyAndSort(
...
): Components.Schemas.WorldShopStandPrice[] {
    ...
    return items
        .slice(0)
        .sort((a: Components.Schemas.WorldShopStandPrice, b: Components.Schemas.WorldShopStandPrice) => {
            if (isItem) {
            ...
            const key = columnKey as keyof Components.Schemas.WorldShopStandPrice;
...
class WorldShopStands extends APIListDisplay {
    ...
    onColumnClick = (event: React.MouseEvent<HTMLElement>, column: IColumn): void => {
        ...
        const items = this.state.results.items as Components.Schemas.WorldShopStandPrice[];
    ...
    getKey = (item: Components.Schemas.WorldShopStandPrice | undefined, index?: number | undefined) => {
        if (item === undefined) {
            if (index === undefined) {
                return "";
            }
...
export const WorldShopStandsDisplay = connector(withRouter(withTranslation()(WorldShopStands)));
